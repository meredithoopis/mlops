...
def find_images_with_multiple_cars(output_file="car_images.txt"):
    """
    ...
    if not os.path.exists(train_annotations_file):
        ...
        os.system(f"wget https://storage.googleapis.com/openimages/v5/train-annotations-bbox.csv")
    ...
    chunk_size = 1000000
    for chunk in tqdm(pd.read_csv(train_annotations_file, chunksize=chunk_size)):
    ...
    # Write the image IDs to a file
    with open(output_file, 'w') as f:
        for img_id in images_with_multiple_cars:
            f.write(f"train/{img_id}\n")
    
    print(f"Image IDs saved to {output_file}")
    print(f"You can now use the original downloader script with this file: python downloader.py {output_file}")

if __name__ == "__main__":
    find_images_with_multiple_cars()